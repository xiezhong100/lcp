...
<link href="${ctx}/css/secondLevelCategory.css" rel="stylesheet" type="text/css" />
...
$(function($) {
	...
	function showHidden(myspan){
		$("#allsort .hidden-item").toggle();
...
<body>
	...
	<div class="w">
		...
		<div id="category-select" class="total">
			所有分类
			<c:forEach var="treeCategory" items="${treeCategorys }">
				 > <a href="${treeCategory.url }">${treeCategory.name}</a>
			</c:forEach>
			...
			<div id="allsort">
				...
				<c:if test="${i >= 10 }">
					<div class="item hidden-item">
						<span><h4><a href="${mycategory.url }">${mycategory.name }</a></h4> </span>
					</div>
				</c:if>
				</c:forEach>
				<%-- 
				<c:if test="${i >= 10 }">
					<div class="extand">
						<span id="allsort-showOther" onclick="showHidden($(this))"  style="cursor:pointer">展开</span>
					</div>
				</c:if>	
				--%>
				</div>
