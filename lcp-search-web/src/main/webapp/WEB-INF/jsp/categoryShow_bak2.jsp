	   		...
	   		<div class="item fore">
	  			...
	  			<div class="i-mc">
	  				<div class='close' onclick="$(this).parent().parent().removeClass('hover')"></div>
		   			<div class="subitem">
		   				<dl><c:set var="count" value="0"></c:set><c:forEach items="${children }" var="child"><c:set var="secondCategory" value="${child.key}" /><c:set var="count" value="${count+1 }"></c:set><c:set var="secondChildren" value="${child.value}" /><c:if test="${secondCategory.leaf }"><c:if test="${count < 80 }">
		   						<em> <a href="${secondCategory.url }">${secondCategory.name }</a> </em> </c:if><c:if test="${count >= 80 }">
		   						<em class="hidden-em"> <a href="${secondCategory.url }">${secondCategory.name }</a> </em></c:if></c:if><c:if test="${!secondCategory.leaf }">
						...
						<dl></c:if></c:forEach>
	    				</dl><c:if test="${count >= 80 }">
	    				<dl class="extand"><span onclick="showHiddenEm($(this))">展开</span></dl></c:if>
	    			</div>
	    		</div>
	  		</div></c:forEach>
  		</div>
